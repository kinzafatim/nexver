...
const Sidebar: React.FC<SidebarProps> = ({ projectState, onFileSelect, onNavigateToTab, isSidebarOpen }) => {
  ...
  const Folder: React.FC<{ name: string; children: React.ReactNode; defaultOpen?: boolean }> = ({ name, children }) => (
    <div>
      <div onClick={() => toggleFolder(name)} className="flex items-center cursor-pointer p-1 rounded hover:bg-secondary-700 dark:hover:bg-primary-800 transition-colors">
        <ChevronRightIcon className={`w-4 h-4 mr-2 transition-transform ${openFolders[name] ? 'rotate-90' : ''}`} />
        <FolderIcon className="w-5 h-5 mr-2 text-secondary-400" />
        <span className="font-semibold">{name}</span>
      </div>
      {openFolders[name] && <div className="pl-6 border-l border-secondary-900 ml-3">{children}</div>}
    </div>
  );
  ...
  return (
    <div className="w-full h-full bg-secondary-800 text-white dark:bg-primary-900 dark:text-secondary-200 p-4 flex flex-col">
      {/* Project Title */}
      <div className="flex-shrink-0 text-lg font-bold text-white dark:text-white border-b border-secondary-700 dark:border-primary-800 pb-2 mb-2">{project.name}</div>
        ...
        <Folder name="Inputs">
            <Folder name="Specification Docs">
                ...
                project.specFiles.map(f => 
                    <div key={f.name} onClick={() => onFileSelect(f)} className="cursor-pointer rounded hover:bg-secondary-700 dark:hover:bg-primary-800">
                        <FileItem name={f.name} type="pdf" />
                    </div>
                )
                ) : (
                <p className="text-xs text-secondary-400 p-1">No files uploaded.</p>
                )}
            </Folder>
        </Folder>
        ...
        {status >= AppStatus.VPGenerated && (
            <Folder name="Outputs">
                <Folder name="Verification Plan">
                    <div onClick={() => onNavigateToTab(ActiveTab.VerificationPlan)} className="cursor-pointer rounded hover:bg-secondary-700 dark:hover:bg-primary-800">
                        <FileItem name="verification_plan.pdf" type="pdf" />
                    </div>
                </Folder>
                 <Folder name="Logs">
                    <div onClick={() => onFileSelect({ name: 'logs.txt', type: 'Log' })} className="cursor-pointer rounded hover:bg-secondary-700 dark:hover:bg-primary-800">
