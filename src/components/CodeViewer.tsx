...
const CodeViewer: React.FC<CodeViewerProps> = ({ code, language = 'systemverilog' }) => {
    ...
    return line.split(/(\s+|[;()\[\]{}])/)
      .map((word, index) => {
        ...
        if (!isNaN(Number(word))) {
          return <span key={index} className="text-secondary-300">{word}</span>;
        }
  ...
  return (
    <div className="bg-primary-900 rounded-lg p-4 font-mono text-sm text-secondary-300 overflow-auto h-full">
      <pre>
        <code>
          {code.split('\n').map((line, i) => (
