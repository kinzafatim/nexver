...
import { ActiveTab, AppStatus, UploadedFile, Project, LogEntry, VerificationPlanRow } from '../types';
...
interface MainContentProps {
  ...
  // Fix: onRunSimulation updated to be parameter-less as state is lifted.
  ...
  // Fix: Add props for simulation state managed by parent.
...
const MainContent: React.FC<MainContentProps> = ({ status, project, logs, activeTab, onTabChange, onFilesUploaded, onGenerateVP, onGenerateTBAndTests, onRunSimulation, onUpdateVPData, previewFile, onClosePreview, exportAction, onExportComplete, isSimulating, simulationProgress }) => {
  ...
  const handleDownloadPDF = useCallback(() => {
    ...
    // FIX: Removed optional chaining for project.name as it's already guarded against being null/undefined.
    let htmlToPrint = `<div style="padding: 2rem; background-color: white; font-family: sans-serif; color: #1e293b;">
        <h1 style="font-size: 1.875rem; font-weight: bold; padding-bottom: 0.5rem; margin-bottom: 1rem; color: #2676d5;">Verification Plan: ${project.name}</h1>`;
    
    // Features Table
    const featuresTableContent = project.verificationPlanData.map(row => `
  ...
  const handleDownloadMD = useCallback(() => {
    ...
    // Features sheet
    if (project.verificationPlanData) {
        mdContent += `## Features\n\n`;
  ...
  // Fix: Combine local isLoading with isSimulating from props for UI state
            ...
            <div style={{ display: activeTab === ActiveTab.VerificationPlan ? 'block' : 'none' }} className="h-full flex flex-col">
                 <PageHeader title="Verification Plan">
                    <span className="text-sm text-secondary-500">
                        Generated on Oct 3, 2025 at 2:30 PM
                    </span>
                    ...
                    <div className="bg-white dark:bg-secondary-900 p-4 rounded-lg shadow">
                        ...
                        <ResponsiveContainer width="100%" height={300}>
                            <BarChart data={coverageData}>
                                ...
                                <Legend />
                                <Bar dataKey="coverage" fill="#2676d5" />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                    <div className="bg-white dark:bg-secondary-900 p-4 rounded-lg shadow">
